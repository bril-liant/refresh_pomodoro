...
SETTINGS_FILENAME = "adhd_pomodoro_settings.json"  # 설정 파일 이름


def get_settings_path():
    """설정 파일 경로를 반환합니다. (OS별 사용자 데이터 폴더 우선)"""
    app_name = "ADHD_Pomodoro"
    ...
    if not os.path.exists(path):
        try:
            os.makedirs(path)
        except OSError:  # 권한 문제 등으로 폴더 생성 실패 시
            # 프로그램 실행 파일과 동일한 위치에 저장 (PyInstaller로 단일 파일 배포 시 유용)
            if getattr(sys, "frozen", False):  # PyInstaller로 빌드된 경우
                path = os.path.dirname(sys.executable)
            else:  # 일반 스크립트 실행 시
                path = os.path.dirname(os.path.abspath(__file__))
            return os.path.join(path, SETTINGS_FILENAME)  # 바로 파일명 반환
...
class PomodoroApp:
    def __init__(self, root_window):
        ...
        self.root.title("ADHD 뽀모도로")
        self.root.configure(bg=COLOR_BACKGROUND)
        self.settings_path = get_settings_path()  # 설정 파일 경로
        ...
        self.load_settings()  # UI 생성 전에 설정 불러오기
        ...
        self.toggle_always_on_top_action()  # 로드된 설정값으로 초기 상태 적용
        self.root.update_idletasks()
        self.adjust_window_size()

        self.root.protocol(
            "WM_DELETE_WINDOW", self.on_closing
        )  # 창 닫기 버튼 클릭 시 on_closing 함수 호출
    ...
    def load_settings(self):
        ...
        try:
            ...
            # 기본값 대신 불러온 값으로 StringVar/BooleanVar 설정
            self.work_minutes_var.set(
                settings.get("work_minutes", self.work_minutes_default)
            )
            self.rest_minutes_var.set(
                settings.get("rest_minutes", self.rest_minutes_default)
            )
            ...
            # 오늘 총 집중 시간 및 뽀모도로 사이클은 저장하지 않고 매일 초기화
            self.total_work_seconds_today = (
                settings.get("total_work_seconds_today", 0)
                if settings.get("last_saved_date") == str(datetime.date.today())
                else 0
            )
            self.pomodoro_cycles_today = (
                settings.get("pomodoro_cycles_today", 0)
                if settings.get("last_saved_date") == str(datetime.date.today())
                else 0
            )
    ...
    def save_settings(self):
        ...
        settings = {
            ...
            "total_work_seconds_today": self.total_work_seconds_today,  # 오늘 통계도 저장
            "pomodoro_cycles_today": self.pomodoro_cycles_today,
            "last_saved_date": str(datetime.date.today()),  # 마지막 저장 날짜
        }
        try:
    ...
    def start_timer(self):
        if self.is_running:
            return
        # work_minutes_var, rest_minutes_var 사용
        ...
        if work_minutes is None or rest_minutes is None:
            return
        if self.use_long_rest_suggestion_var.get():
            threshold = self.validate_cycle_input(
                self.long_rest_cycle_threshold_var.get(), "긴 휴식 반복 횟수"
            )
            duration = self.validate_time_input(
        ...
        if self.use_long_rest_suggestion_var.get():
            entry_list_to_disable.extend(
                ["long_rest_cycle_entry", "long_rest_duration_entry"]
            )
        for entry_var_name in entry_list_to_disable:
            entry_widget = getattr(self, entry_var_name, None)
            if entry_widget:
    ...
    def countdown(self):
        ...
        if self.remaining_seconds == 0:
            if self.current_mode == "집중":
                ...
                if (
                    self.use_long_rest_suggestion_var.get()
                    and long_rest_threshold is not None
                    ...
                    if messagebox.askyesno(
                        ...
                        self.update_timer_display()
                        self.show_overlay_window(long_rest_duration, is_long_rest=True)
                        return
            self.switch_mode()
        else:
    ...
    def stop_timer(self):
        ...
        for entry_var_name in entry_list_to_enable:
            entry_widget = getattr(self, entry_var_name, None)
            if entry_widget:
    ...
    def check_meal_time_periodically(self):
        ...
        if current_date != self.today_date:
            ...
            self.total_work_seconds_today = 0
            self.pomodoro_cycles_today = 0
            self.update_stats_display()  # 날짜 변경 시 통계 업데이트
